...
export default function BroadcastPage() {
  ...
  const handleBroadcast = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || 'Failed to queue broadcast');
    } finally {
      setIsSending(false);
    }
  };
  ...
  return (
    <motion.div variants={containerVars} initial="hidden" animate="show" className="max-w-6xl mx-auto flex flex-col gap-8 pb-24 font-['Helvena']">
      
      <div>
        <h1 className={`text-3xl sm:text-4xl font-['Libre_Baskerville'] italic ${textColor} flex items-center gap-3`}>
          <Megaphone size={32} className="text-[#FE6100]" /> Communication Center
        </h1>
        <p className={`text-sm sm:text-base mt-2 ${subTextColor}`}>Queue emails and WhatsApp messages to your students via Redis.</p>
      </div>
      ...
      <form onSubmit={handleBroadcast} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 sm:gap-8">
          ...
          <div className={`p-5 sm:p-6 rounded-2xl backdrop-blur-xl border shadow-sm flex flex-col gap-4 transition-colors duration-500 ${cardBg}`}>
            ...
            <div className="flex flex-col gap-3">
              <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${target === 'all' ? 'border-[#FE6100] bg-[#FE6100]/5' : `${borderColor} hover:border-zinc-400 ${isCoding ? 'bg-black/20' : 'bg-zinc-50'}`}`}>
                <input type="radio" name="target" checked={target === 'all'} onChange={() => setTarget('all')} className="accent-[#FE6100] w-4 h-4" />
              ...
              <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${target === 'course' ? 'border-[#FE6100] bg-[#FE6100]/5' : `${borderColor} hover:border-zinc-400 ${isCoding ? 'bg-black/20' : 'bg-zinc-50'}`}`}>
                <input type="radio" name="target" checked={target === 'course'} onChange={() => setTarget('course')} className="accent-[#FE6100] w-4 h-4" />
              ...
              <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${target === 'specific' ? 'border-[#FE6100] bg-[#FE6100]/5' : `${borderColor} hover:border-zinc-400 ${isCoding ? 'bg-black/20' : 'bg-zinc-50'}`}`}>
                <input type="radio" name="target" checked={target === 'specific'} onChange={() => setTarget('specific')} className="accent-[#FE6100] w-4 h-4" />
            ...
            <AnimatePresence mode="wait">
              {target === 'course' && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-2 mt-2">
                  ...
                  <input type="text" required value={courseId} onChange={(e) => setCourseId(e.target.value)} placeholder="Paste Course ID..." className={`w-full h-10 px-3 rounded-lg border outline-none text-sm transition-all ${inputBg}`} />
                </motion.div>
              )}
              {target === 'specific' && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-2 mt-2">
                  <label className={`text-xs font-medium uppercase tracking-wider ${subTextColor}`}>User ID</label>
                  <input type="text" required value={specificUserId} onChange={(e) => setSpecificUserId(e.target.value)} placeholder="Paste User ID..." className={`w-full h-10 px-3 rounded-lg border outline-none text-sm transition-all ${inputBg}`} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className={`p-5 sm:p-6 rounded-2xl backdrop-blur-xl border shadow-sm flex flex-col gap-4 transition-colors duration-500 ${cardBg}`}>
            ...
            <div className="flex flex-col gap-3">
              <div onClick={() => toggleChannel('email')} className={`flex items-center justify-between p-4 rounded-lg border cursor-pointer transition-all ${channels.email ? 'border-blue-500 bg-blue-500/5' : `${borderColor} hover:border-zinc-400 ${isCoding ? 'bg-black/20' : 'bg-zinc-50'}`}`}>
                ...
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${channels.email ? 'border-blue-500 bg-blue-500' : 'border-zinc-400'}`}>
              ...
              <div onClick={() => toggleChannel('whatsapp')} className={`flex items-center justify-between p-4 rounded-lg border cursor-pointer transition-all ${channels.whatsapp ? 'border-green-500 bg-green-500/5' : `${borderColor} hover:border-zinc-400 ${isCoding ? 'bg-black/20' : 'bg-zinc-50'}`}`}>
                ...
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${channels.whatsapp ? 'border-green-500 bg-green-500' : 'border-zinc-400'}`}>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6 sm:gap-8">
          ...
          <div className={`p-5 sm:p-8 rounded-2xl backdrop-blur-xl border shadow-sm flex flex-col gap-6 transition-colors duration-500 ${cardBg}`}>
            ...
            {channels.email && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-5 pb-6 border-b border-zinc-500/20">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="e.g. Important Update Regarding Your Course" className={`w-full h-12 px-4 rounded-xl border outline-none text-base transition-all ${inputBg}`} />
                </div>

                <div className="flex flex-col gap-2">
                  <label className={`text-xs font-medium uppercase tracking-wider ${subTextColor}`}>HTML Message Body</label>
                  <textarea rows={8} value={htmlMessage} onChange={(e) => setHtmlMessage(e.target.value)} placeholder="<p>Write your raw HTML email template here...</p>" className={`w-full p-4 rounded-xl border outline-none font-mono text-sm resize-none transition-all ${inputBg}`} />
                  <p className={`text-xs mt-1 ${subTextColor}`}>Input raw HTML. Inline styles are supported.</p>
                </div>
              </motion.div>
            )}

            {channels.whatsapp && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`flex flex-col gap-5 ${channels.email ? 'pt-2' : ''}`}>
                <h2 className={`text-xl font-bold flex items-center gap-2 ${textColor}`}>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea rows={5} value={textMessage} onChange={(e) => setTextMessage(e.target.value)} placeholder="Write your WhatsApp alert here..." className={`w-full p-4 rounded-xl border outline-none text-base resize-none transition-all ${inputBg}`} />
                </div>
              </motion.div>
            )}

            {!channels.email && !channels.whatsapp && (
              <div className={`py-12 text-center ${subTextColor}`}>
      ...
      <motion.div 
        ...
        className={`fixed bottom-0 lg:bottom-8 left-0 lg:left-1/2 lg:-translate-x-1/2 w-full lg:max-w-4xl p-4 lg:rounded-2xl border-t lg:border shadow-[0_-10px_40px_rgba(0,0,0,0.1)] flex justify-between items-center backdrop-blur-2xl z-50 transition-colors duration-500 ${cardBg}`}
      >
        <p className={`hidden sm:block text-sm font-medium ${subTextColor}`}>Double-check your target audience before sending.</p>
        <button 
          onClick={handleBroadcast}
          disabled={isSending || (!channels.email && !channels.whatsapp)}
          className="w-full sm:w-auto h-12 px-8 bg-gradient-to-r from-[#FE6100] to-[#FC3500] rounded-xl text-white font-bold tracking-wide hover:shadow-[0_0_15px_rgba(254,97,0,0.5)] hover:scale-105 transition-all disabled:opacity-50 disabled:hover:scale-100 flex justify-center items-center gap-2"
        >
          {isSending ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/> Queueing...</> : <><Send size={18} /> Launch Broadcast</>}
        </button>
      </motion.div>
