...
export default function CourseManagerPage() {
  ...
  const [deleteConfig, setDeleteConfig] = useState<{
    isOpen: boolean;
    type: "lecture" | "faq" | "resource" | null;
    idStr: string;
    resourceType?: "notes" | "pyqs";
  }>({
    isOpen: false,
    type: null,
    idStr: "",
  ...
  const [uploadMethod, setUploadMethod] = useState<"upload" | "reuse">(
    "upload"
  );
    ...
    try {
      ...
      if (data.success) {
        ...
        setEditForm({
          ...
          mrp: c.mrp?.toString() || "0",
          validityInDays: c.validityInDays?.toString() || "365",
          mentorName: c.mentorName || "",
          mentorDescription: c.mentorDescription || "",
          tags: c.tags?.join(", ") || "",
        ...
        if (c.lectures && c.lectures.length > 0) {
          const folders = new Set(
            c.lectures.map((l: any) => l.folderName || "General")
          );
  ...
  const handleUploadLecture = async (e: React.FormEvent) => {
    ...
    if (uploadMethod === "reuse" && !lectureForm.reuseVideoUrl)
      return showToast("Please select a video from the library.", "error");
    ...
    try {
      ...
      if (uploadMethod === "upload") {
        ...
        if (isLegacyHttp) payload.videoUrl = finalUid;
        else payload.reuseCloudflareUid = finalUid;
      }
  ...
  const handleUpdateSettings = async () => {
    ...
    try {
      ...
      formData.append(
        "validityInDays",
        String(Number(editForm.validityInDays))
      );
      ...
      if (course.domain === "university") {
        formData.append("semester", String(Number(editForm.semester)));
      ...
      if (editForm.newFaqQ && editForm.newFaqA) {
        const newFaqs = [
          ...(course.faqs || []),
          { question: editForm.newFaqQ, answer: editForm.newFaqA },
        ];
        formData.append("faqs", JSON.stringify(newFaqs));
      } else {
        formData.append("faqs", JSON.stringify(course.faqs || []));
      }
    ...
    try {
      if (type === "lecture") {
        ...
        await api.put(`/courses/${courseId}`, {
          faqs: JSON.stringify(newFaqs),
        });
      } else if (type === "resource" && resourceType) {
        await api.delete(
          `/courses/${courseId}/resources/${idStr}?type=${resourceType}`
        );
      }
  ...
  const filteredLibrary = mediaLibrary.filter(
    (item) =>
      ...
      (item.course?.title &&
        item.course.title.toLowerCase().includes(librarySearch.toLowerCase()))
  );
  ...
  const inputBg = isCoding
    ? "bg-black/50 border-zinc-800 text-white focus:border-[#FE6100]"
    : "bg-white border-zinc-300 text-stone-900 focus:border-[#FE6100]";
  ...
  if (pageError || !course)
    return (
      <div className="max-w-6xl mx-auto flex flex-col items-center justify-center min-h-[50vh] gap-4">
        ...
        <h2 className={`text-2xl font-bold ${textColor}`}>{pageError}</h2>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto flex flex-col gap-8 pb-24 relative font-['Helvena']">
          ...
          <>
            ...
            >
              ...
              <p className={`text-sm ${subTextColor}`}>
                Are you sure you want to delete this {deleteConfig.type}? This
      ...
      <div className="flex flex-col gap-4">
        <Link
          ...
          className={`flex items-center gap-2 text-sm w-fit hover:text-[#FE6100] transition-colors ${subTextColor}`}
        >
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
          <div>
            <div className="flex items-center flex-wrap gap-3 mb-2">
              ...
              {course.domain === "university" && (
                <span className={`text-xs font-semibold ${subTextColor}`}>
                  {course.university?.name || course.universityName} • Sem{" "}
            ...
            <h1
              className={`text-3xl sm:text-4xl font-['Libre_Baskerville'] italic ${textColor}`}
            >
              {course.title}
            </h1>
            <p className={`text-sm mt-2 ${subTextColor}`}>
              ₹{course.price}{" "}
              {course.mrp > course.price && `(MRP: ₹${course.mrp})`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-zinc-500/20 pb-2 overflow-x-auto no-scrollbar">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors whitespace-nowrap ${
            activeTab === "lectures"
              ? "bg-[#FE6100]/10 text-[#FE6100] border-b-2 border-[#FE6100]"
              : `${subTextColor} hover:bg-zinc-500/10`
          }`}
        >
          Video Lectures
        </button>
      ...
      <div
        className={`rounded-2xl backdrop-blur-xl border shadow-sm flex flex-col overflow-hidden transition-colors duration-500 ${cardBg}`}
      >
        {activeTab === "lectures" && (
          <>
            <div
              className={`p-5 sm:p-6 border-b ${borderColor} flex justify-between items-center gap-3`}
            >
              ...
              <button
                ...
                className="h-10 px-4 bg-[#FE6100] rounded-lg text-white text-sm font-medium flex items-center gap-2"
              >
            ...
            <div className="flex flex-col">
              {Object.keys(groupedLectures).length === 0 ? (
                <div className={`p-12 text-center ${subTextColor}`}>
                  No lectures uploaded yet. Click "Add Lecture" to build your
                  syllabus.
                </div>
              ) : (
                Object.keys(groupedLectures).map((folderName) => (
                  ...
                  >
                    <div
                      className={`px-6 py-3 bg-zinc-500/5 text-sm font-bold tracking-wider uppercase ${subTextColor}`}
                    >
                      {folderName}
                    </div>
                    {groupedLectures[folderName]
                      .sort((a: any, b: any) => a.sequence - b.sequence)
                      .map((lecture: any) => (
                        <div
                          key={lecture._id}
                          className={`p-4 sm:p-6 border-b last:border-0 flex justify-between items-center group hover:bg-zinc-500/5 ${borderColor}`}
                        >
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs bg-[#FE6100]/10 text-[#FE6100]`}
                            >
                              {lecture.sequence}
                            </div>
                            <div>
                              ...
                              <p
                                className={`text-xs mt-0.5 ${subTextColor} line-clamp-1`}
                              >
                                {lecture.description}
                              </p>
                            </div>
                          ...
                          <div className="flex gap-1">
                            ...
                            <button
                              onClick={() =>
                                openDeleteModal("lecture", lecture._id)
                              }
        ...
        {activeTab === "resources" && (
          <>
            ...
            >
              <div className="flex items-center gap-2">
                <FileText className={textColor} size={20} />{" "}
                <h2 className={`text-lg font-bold ${textColor}`}>
                  Study Materials
                </h2>
              </div>
              <button
                ...
                className="h-10 px-4 bg-blue-600 hover:bg-blue-700 transition-colors rounded-lg text-white text-sm font-medium flex items-center gap-2"
              >
            ...
            <div className="p-6">
              <h3
                className={`text-sm font-bold uppercase tracking-wider mb-4 ${textColor}`}
              >
                Course Notes
              </h3>
              {course.notes?.length > 0 ? (
                course.notes.map((note: any) => (
                  <div
                    key={note._id}
                    className={`p-4 mb-2 rounded-lg border ${borderColor} flex justify-between items-center group hover:bg-zinc-500/5 transition-colors`}
                  >
                    <div className="flex items-center gap-3">
                      <FileText size={16} className="text-blue-500" />
                      <span className={textColor}>{note.title}</span>
                    ...
                    <button
                      onClick={() =>
                        openDeleteModal("resource", note._id, "notes")
                      }
                      className="p-2 text-zinc-400 hover:text-red-500 transition-colors"
                    >
                      <Trash2 size={16} />
              ...
              <h3
                className={`text-sm font-bold uppercase tracking-wider mb-4 mt-8 ${textColor}`}
              >
                Previous Year Questions (PYQs)
              </h3>
              {course.pyqs?.length > 0 ? (
                course.pyqs.map((pyq: any) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className={textColor}>{pyq.title}</span>
                      <span className={`text-xs ${subTextColor}`}>
                        ({pyq.fileSize})
                      </span>
                    </div>
                    <button
                      onClick={() =>
                        openDeleteModal("resource", pyq._id, "pyqs")
                      }
                      className="p-2 text-zinc-400 hover:text-red-500 transition-colors"
                    >
                      <Trash2 size={16} />
        ...
        {activeTab === "settings" && (
          <div className="p-6 flex flex-col gap-8">
            <div
              className={`p-5 rounded-xl border flex flex-col gap-5 ${borderColor}`}
            >
              <div className="flex justify-between items-center">
                ...
                <div className="relative">
                  ...
                  {newThumbnail && (
                    <span className="absolute -top-6 right-0 text-[10px] text-green-500 font-bold whitespace-nowrap">
                      File ready to save
                    </span>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  className={`p-3 rounded-lg border outline-none resize-none transition-all ${inputBg}`}
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    type="number"
                    value={editForm.validityInDays}
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        validityInDays: e.target.value,
                      })
                    }
            ...
            >
              <h3
                className={`text-sm font-bold uppercase tracking-wider ${textColor}`}
              >
                Domain Specifics ({course.domain})
              </h3>
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="text"
                  placeholder="Job Ready, MERN Stack"
                  value={editForm.tags}
                  onChange={(e) =>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="text"
                  placeholder="100% Syllabus Covered, Doubt Support"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, mentorName: e.target.value })
                    }
                    className={`h-11 px-4 rounded-lg border outline-none transition-all ${inputBg}`}
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    type="text"
                    value={editForm.mentorDescription}
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
            ...
            <div className={`border-t ${borderColor} pt-6`}>
              <h3
                className={`text-lg font-bold flex items-center gap-2 mb-4 ${textColor}`}
              >
                <MessageCircle size={18} /> Manage FAQs
              </h3>

              {course.faqs?.map((faq: any) => (
                <div
                  key={faq._id}
                  className={`p-4 rounded-lg border mb-3 flex justify-between items-start ${borderColor}`}
                >
                  ...
                  <button
                    onClick={() => openDeleteModal("faq", faq._id)}
                    className="p-2 text-zinc-400 hover:text-red-500 transition-colors"
                  >
                    <Trash2 size={16} />
                  </button>
              ...
              <div
                className={`p-5 rounded-xl border mt-4 ${borderColor} ${
                  isCoding ? "bg-black/20" : "bg-zinc-50"
                }`}
              >
                <h4
                  className={`text-sm font-bold uppercase tracking-wider mb-4 ${textColor}`}
                ...
                <div className="flex flex-col gap-3">
                  <input
                    ...
                    value={editForm.newFaqQ}
                    onChange={(e) =>
                      setEditForm({ ...editForm, newFaqQ: e.target.value })
                    }
                    className={`h-11 px-4 rounded-lg border outline-none transition-all focus:border-[#FE6100] ${inputBg}`}
                  />
                  <textarea
                    placeholder="Answer..."
                    rows={2}
                    value={editForm.newFaqA}
                    onChange={(e) =>
                      setEditForm({ ...editForm, newFaqA: e.target.value })
                    }
                    className={`p-4 rounded-lg border outline-none resize-none transition-all focus:border-[#FE6100] ${inputBg}`}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end sticky bottom-0 py-4 bg-inherit z-10">
              <button
                ...
                disabled={isUploading}
                className="px-8 h-12 bg-[#FE6100] hover:bg-orange-700 transition-colors rounded-xl text-white font-medium shadow-lg disabled:opacity-50 flex items-center gap-2"
              >
                {isUploading ? (
        ...
        {isEditLectureModalOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isUploading && setIsEditLectureModalOpen(false)}
              ...
              >
                ...
                <div className="flex gap-4">
                  ...
                  <div className="w-24 flex flex-col gap-1">
                    ...
                    <input
                      type="number"
                      required
                      value={editLectureForm.sequence}
                      onChange={(e) =>
                        setEditLectureForm({
                          ...editLectureForm,
                      ...
                      className={`h-11 px-4 rounded-lg border outline-none text-center transition-all ${inputBg}`}
                    />
                  </div>
                </div>
                <div
                  className={`p-4 rounded-lg border flex justify-between items-center ${inputBg}`}
                >
                  <div>
                    <h4
                      className={`text-sm font-medium ${textColor} flex items-center gap-2`}
                    >
                      {editLectureForm.isFree ? (
                  ...
                  <button
                    ...
                    onClick={() =>
                      setEditLectureForm({
                        ...editLectureForm,
                        isFree: !editLectureForm.isFree,
                      })
                    }
                    className={`w-12 h-6 rounded-full p-1 transition-colors ${
                      editLectureForm.isFree ? "bg-green-500" : "bg-zinc-500"
                    }`}
                  >
                    <div
                ...
                <div className="flex justify-end gap-3 mt-4">
                  <button
                    ...
                    disabled={isUploading}
                    className={`px-6 h-11 rounded-lg text-sm font-medium transition-colors hover:bg-zinc-500/10 ${textColor}`}
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={isUploading}
                    className="px-6 h-11 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"
                  >
                    {isUploading ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (
        ...
        {isLectureModalOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isUploading && setIsLectureModalOpen(false)}
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className={`fixed top-[5%] sm:top-[10%] left-1/2 -translate-x-1/2 w-[95%] sm:w-full max-w-2xl p-6 sm:p-8 rounded-2xl border shadow-2xl z-50 flex flex-col gap-6 max-h-[90vh] overflow-y-auto ${modalBg}`}
            >
              <h2
                className={`text-2xl font-['Libre_Baskerville'] italic ${textColor}`}
              >
              ...
              >
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedFolder(
                          !isNewFolder ? "" : existingFolders[0] || "General"
                        );
                      }}
                      className="text-xs text-[#FE6100] hover:underline"
                    >
                ...
                >
                  <div className="flex gap-2 p-1 bg-zinc-500/10 rounded-lg w-fit mx-auto">
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${
                        uploadMethod === "upload"
                          ? "bg-[#FE6100] text-white shadow"
                          : `${subTextColor} hover:text-stone-900 dark:hover:text-white`
                      }`}
                    >
                      Upload New File
                    </button>
                  ...
                  {uploadMethod === "upload" ? (
                    <div
                      className={`relative w-full h-24 rounded-lg border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-500/5 transition-colors ${
                        videoFile ? "border-[#FE6100]" : borderColor
                      }`}
                    >
                      <input
                        type="file"
                      ...
                      />
                      <UploadCloud
                        className={videoFile ? "text-[#FE6100]" : subTextColor}
                        size={24}
                      />
                      <span
                        className={`text-sm mt-2 font-medium truncate px-4 ${
                          videoFile ? "text-[#FE6100]" : textColor
                        }`}
                      >
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className={`w-full h-10 pl-9 pr-4 rounded-lg border outline-none text-sm transition-all ${inputBg}`}
                        />
                      </div>
                      <div
                        className={`h-40 overflow-y-auto rounded-lg border p-2 flex flex-col gap-2 ${
                          isCoding
                      ...
                      >
                        {filteredLibrary.length === 0 ? (
                          <div
                            className={`flex justify-center items-center h-full text-xs font-medium ${subTextColor}`}
                          >
                          ...
                          filteredLibrary.map((item, idx) => {
                            ...
                            const isSelected =
                              lectureForm.reuseVideoUrl === uidToUse;
                            return (
                              <div
                                key={idx}
                                onClick={() =>
                              ...
                              >
                                <div className="flex flex-col truncate pr-4">
                                  <span
                                    className={`text-sm font-medium truncate ${
                                      isSelected ? "text-[#FE6100]" : textColor
                                    }`}
                                  >
                                    {item.title}
                                  </span>
                                  <span
                                    className={`text-[10px] uppercase tracking-wider mt-0.5 ${subTextColor}`}
                                  >
                                    From:{" "}
                                ...
                                {isSelected && (
                                  <CheckCircle2
                                    className="text-[#FE6100] shrink-0"
                                    size={18}
                                  />
                                )}
                              </div>
                ...
                >
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full p-1 transition-colors ${
                      lectureForm.isFree ? "bg-green-500" : "bg-zinc-500"
                    }`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full bg-white transition-transform ${
                        lectureForm.isFree ? "translate-x-6" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-3 mt-2">
                    ...
                    <div className="w-full flex flex-col gap-1.5">
                      <div className="flex justify-between items-center text-xs font-bold">
                        <span className={textColor}>
                          {uploadProgress < 100
                            ? "Uploading direct to Cloudflare..."
                            : "Finalizing metadata..."}
                        </span>
                        <span className="text-[#FE6100]">
                          {uploadProgress}%
                        </span>
                      </div>
                  ...
                  <div className="flex flex-col-reverse sm:flex-row justify-end gap-3">
                    <button
                      ...
                      disabled={isUploading}
                      className={`w-full sm:w-auto px-6 h-11 rounded-lg text-sm font-medium transition-colors hover:bg-zinc-500/10 disabled:opacity-50 ${textColor}`}
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      disabled={isUploading}
                      className="w-full sm:w-auto px-6 h-11 bg-gradient-to-r from-[#FE6100] to-[#FC3500] rounded-lg text-white text-sm font-medium hover:opacity-90 flex justify-center items-center gap-2 disabled:opacity-50"
                    >
                      {isUploading ? (
                        <>
        ...
        {isResourceModalOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isUploading && setIsResourceModalOpen(false)}
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className={`fixed top-[5%] sm:top-[15%] left-1/2 -translate-x-1/2 w-[95%] sm:w-full max-w-lg p-6 sm:p-8 rounded-2xl border shadow-2xl z-50 flex flex-col gap-6 ${modalBg}`}
            >
              <h2
                className={`text-2xl font-['Libre_Baskerville'] italic ${textColor}`}
              >
              ...
              >
                ...
                <div
                  className={`relative w-full h-24 mt-2 rounded-lg border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-500/5 transition-colors ${
                    resourceFile ? "border-blue-500" : borderColor
                  }`}
                >
                  <input
                    type="file"
                  ...
                  />
                  <FileText
                    className={resourceFile ? "text-blue-500" : subTextColor}
                    size={24}
                  />
                  <span
                    className={`text-sm mt-2 font-medium truncate px-4 w-full text-center ${
                      resourceFile ? "text-blue-500" : textColor
                    }`}
                  >
                    {resourceFile ? resourceFile.name : "Select PDF File"}
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-3 mt-4">
                  ...
                  <div className="flex flex-col-reverse sm:flex-row justify-end gap-3">
                    ...
                    <button
                      type="submit"
                      disabled={isUploading}
                      className="w-full sm:w-auto px-6 h-11 bg-blue-600 hover:bg-blue-700 transition-colors rounded-lg text-white text-sm font-medium flex justify-center items-center gap-2 disabled:opacity-50"
                    >
                      {isUploading ? (
                        <>
                          <Loader2 className="w-4 h-4 animate-spin text-white" />
