                ...
                <div className="relative">
                  ...
                  >
                    <div className="flex justify-start items-center gap-1.5 sm:gap-2">
                      {user?.avatar ? (
                        <img className="w-7 h-7 sm:w-8 sm:h-8 rounded-full object-cover" src={user.avatar} alt="Profile" />
                      ) : (
                  ...
                  <AnimatePresence>
                    {isDropdownOpen && (
                      <motion.div 
                        ...
                        className={`absolute right-0 top-12 w-48 rounded-lg shadow-lg outline outline-1 outline-offset-[-1px] flex flex-col justify-start items-start overflow-hidden z-50 ${menuBg}`}
                      >
                  ...
                  <div className="relative">
                    <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`h-9 pl-1 pr-3 py-1.5 rounded-[100px] shadow-sm outline outline-1 outline-offset-[-1px] ${isCoding ? 'outline-white/20 bg-white/5 hover:bg-white/10' : 'outline-black/10 bg-white hover:bg-gray-50'} inline-flex justify-center items-center gap-2 overflow-hidden transition-colors`}>
                      <div className="flex justify-start items-center gap-2">
                        {user?.avatar ? (
                          <img className="w-7 h-7 rounded-full object-cover" src={user.avatar} alt="Profile" />
                        ) : (
                    ...
                    <AnimatePresence>
                      {isDropdownOpen && (
                        <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.2 }} className={`absolute right-0 top-12 w-48 rounded-lg shadow-lg outline outline-1 outline-offset-[-1px] flex flex-col justify-start items-start overflow-hidden z-50 ${menuBg}`}>
                          <div className={`self-stretch p-1 border-b flex flex-col justify-start items-start ${isCoding ? 'border-white/10' : 'border-neutral-200'}`}>
                            {user?.role === 'admin' && (
                              <Link href="/dashboard" onClick={() => setIsDropdownOpen(false)} className={`self-stretch h-9 p-2 rounded-md inline-flex justify-start items-center gap-3 transition-colors ${menuHover}`}>
                                <LayoutDashboard className={`w-4 h-4 ${iconColor}`} />
