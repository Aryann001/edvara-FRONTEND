...
export default function CourseSelection() {
  ...
  const handleSemesterSelect = (sem: string | number) => {
    const semNumber =
      typeof sem === "string" ? parseInt(sem.split(" ")[0]) : sem;
  ...
  const fetchSubjects = async (uniId: string, branch: string, sem: number) => {
    ...
    try {
      const queryUrl = `/courses?domain=university&university=${uniId}&branch=${encodeURIComponent(
        branch
      )}&semester=${sem}`;
        ...
        <div className="w-full max-w-[1440px] px-5 md:px-10 lg:px-20 py-14 lg:py-24 flex flex-col justify-center items-center gap-10 lg:gap-14">
          <div className="self-stretch flex flex-col justify-center items-center gap-2">
            ...
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1, ease: customEase, duration: 0.6 }}
          ...
          {codingCourses.length > 0 ? (
              ...
              {codingCourses.map((course, idx) => {
                ...
                return (
                  <Link
                    ...
                    className="flex flex-col group outline-none cursor-pointer"
                  >
                    <motion.div
                      ...
                      <div className="self-stretch flex flex-col justify-start items-start gap-8 lg:gap-10 z-10 h-full w-full">
                          ...
                          <div className="self-stretch flex flex-col justify-start items-start gap-4 lg:gap-5 w-full">
                            <div className="self-stretch flex justify-start items-start gap-2 flex-wrap w-full">
                              {course.tags?.map((tag: string) => (
                                <div
                                  key={tag}
                                  className="px-3 py-1.5 bg-white/10 rounded-[100px] flex flex-col justify-start items-start backdrop-blur-sm"
                                >
                                  <div className="text-center text-gray-200 text-xs lg:text-sm font-normal font-['Helvena']">
                                    {tag}
                                  </div>
                                </div>
                              ))}
                        ...
                        <div className="self-stretch flex flex-col justify-end items-start gap-5 mt-auto w-full">
                          <div className="flex flex-col justify-center items-start gap-1 w-full">
                            <div className="text-white text-2xl lg:text-3xl font-medium font-sans">
                              ₹{course.price}
                            </div>
                            {course.mrp > course.price && (
                              <div className="flex items-center flex-wrap gap-1.5 lg:gap-2">
                                <span className="text-white/60 text-xs lg:text-sm font-normal font-['Helvena'] line-through">
                                  MRP ₹{course.mrp}
                                </span>
                                <span className="text-[#FE6100] text-xs lg:text-sm font-semibold font-['Helvena']">
                                  ({discountPercent}% off)
                                </span>
                              </div>
                            )}
                          ...
                          <div className="self-stretch flex justify-start items-start gap-3 lg:gap-4 w-full">
                            <button className="flex-1 w-full h-11 px-4 lg:px-6 bg-[#FE6100] hover:bg-orange-700 transition-colors rounded-lg flex justify-center items-center gap-2 shadow-md cursor-pointer">
                              <span className="text-white text-base font-medium font-['Helvena'] whitespace-nowrap">
                                Check Course
                              </span>
                              <ArrowRight
            ...
            <div className="w-full flex flex-col items-center justify-center py-20 opacity-70">
              ...
              <h3 className="text-white text-xl md:text-2xl font-medium font-['Helvena']">
                New cohorts launching soon.
              </h3>
              <p className="text-gray-400 text-center mt-2">
                Stay tuned for upcoming AI and Full Stack programs.
              </p>
            </div>
          )}
        </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <motion.div
                    ...
                    className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-sm md:text-base font-bold transition-all duration-500 shadow-sm ${
                      step >= num
                        ? "bg-indigo-600 text-white outline outline-2 outline-indigo-600 outline-offset-2"
                        : "bg-white text-gray-400 outline outline-1 outline-gray-200"
                    }`}
                  >
                    {step > num ? (
                      <Check
                  ...
                  <span
                    className={`text-sm md:text-base font-['Helvena'] transition-colors duration-300 ${
                      step === num
                        ? "inline text-indigo-700 font-bold"
                        : "hidden md:inline text-gray-400 font-medium"
                    }`}
                  >
                    {stepNames[idx]}
                  </span>
                        ...
                        {universities.map((uni) => (
                          <div
                            ...
                            className={`relative p-4 md:p-6 rounded-2xl md:rounded-[28px] outline transition-all duration-300 ease-out flex flex-col gap-6 md:gap-8 group cursor-pointer ${
                              !uni.isReady
                                ? "opacity-70 hover:opacity-100 outline-1 outline-gray-200 bg-gray-50 hover:shadow-md"
                          ...
                          >
                            <div className="w-12 h-12 md:w-16 md:h-16 bg-neutral-50 flex items-center justify-center rounded-xl md:rounded-2xl outline outline-1 outline-gray-100 shadow-sm overflow-hidden">
                              <img
                                src={uni.logo?.url || "/placeholder.png"}
                                alt="logo"
                                className="w-8 h-8 md:w-10 md:h-10 object-contain"
                              />
                            </div>
                            <div className="flex flex-col gap-1.5 pr-4">
                              <h4 className="text-neutral-950 text-base md:text-lg font-bold font-['Helvena'] line-clamp-2 leading-snug">
                                {uni.name}
                              </h4>
                              <p className="text-gray-500 font-['Helvena'] text-xs md:text-sm font-medium">
                                {uni.location}
                              </p>
                              <p
                                className={`${
                                  uni.isReady
                                    ? "text-[#FE6100]"
                                    : "text-gray-500"
                                } font-['Helvena'] text-[11px] md:text-sm font-bold tracking-wide mt-1 md:mt-2`}
                              >
                                {uni.isReady ? (
                                  ...
                                  uni.stats ||
                                  `Notify me when ${uni.name} is ready`
                                )}
                              </p>
                            </div>
                            <AnimatePresence>
                              {lastSelectedId === uni._id && (
                                ...
                                >
                                  <Check
                                    className="w-5 h-5 md:w-6 md:h-6 text-[#FE6100]"
                                    strokeWidth={3}
                                  />
                                </motion.div>
                              )}
                          ...
                          >
                            ...
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
                                lastSelectedId === branch
                                  ? "bg-[#FE6100]"
                                  : "bg-neutral-100 group-hover:bg-[#FE6100]/10"
                              }`}
                            >
                              <ArrowRight
                                className={`w-4 h-4 md:w-5 md:h-5 transition-colors ${
                                  lastSelectedId === branch
                                    ? "text-white"
                                    : "text-gray-400 group-hover:text-[#FE6100]"
                                }`}
                                strokeWidth={3}
                              />
                            </div>
                      ...
                      {semesters.map((sem) => (
                        <button
                          ...
                          className={`py-8 md:py-12 rounded-2xl md:rounded-[28px] outline outline-1 outline-gray-200 hover:outline-[#FE6100]/30 hover:shadow-lg font-['Helvena'] text-2xl md:text-4xl font-bold transition-all duration-300 cursor-pointer ${
                            lastSelectedId === sem.toString()
                              ? "outline-2 outline-[#FE6100] bg-[#FE6100]/5 text-[#FE6100] shadow-md"
                              : "bg-white text-neutral-900"
                          }`}
                        >
                          {sem.split(" ")[0]}
                          <span className="block text-xs md:text-sm font-semibold text-gray-400 mt-2 tracking-widest uppercase">
                            Semester
                          </span>
                        </button>
                      ))}
                    ...
                    <div className="w-full min-h-[300px]">
                      {isFetchingSubjects ? (
                        <div className="flex flex-col items-center justify-center py-20 h-full">
                          ...
                          >
                            <div className="w-16 h-16 bg-[#FE6100]/10 rounded-full flex items-center justify-center">
                              <Loader2
                                className="w-8 h-8 text-[#FE6100] animate-spin"
                                strokeWidth={3}
                              />
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {fetchedSubjects.map((course, idx) => {
                            const discountPercent =
                              course.mrp > course.price
                                ? Math.round(
                                    ((course.mrp - course.price) / course.mrp) *
                                      100
                                  )
                                : 0;
                            return (
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className="p-5 flex flex-col flex-1">
                                    <h3 className="text-gray-900 font-bold text-lg leading-tight line-clamp-2 mb-2 font-['Helvena']">
                                      {course.title}
                                    </h3>
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                          ...
                          <h3 className="text-gray-900 text-2xl font-bold font-['Helvena']">
                            Syllabus arriving soon
                          </h3>
                          <p className="text-gray-500 mt-2 max-w-md">
                            We are currently preparing the exact modules for{" "}
                            {selectedUni?.name} - {selectedBranch} (Sem{" "}
                            {selectedSemester}).
                          </p>
                          <button
                            onClick={() => setIsRequestPopupOpen(true)}
